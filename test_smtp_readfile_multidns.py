...
#dns_server_list = ['209.244.0.3', '8.8.8.8', '84.200.69.80', '8.26.56.26', '208.67.222.222', '156.154.70.1', '199.85.126.10', '81.218.119.11', '195.46.39.39', '107.150.40.234', '208.76.50.50', '216.146.35.35', '37.235.1.174', '89.233.43.71', '74.82.42.42', '109.69.8.51']
dns_server_list = ['209.244.0.3', '84.200.69.80', '8.26.56.26', '208.67.222.222', '156.154.70.1', '199.85.126.10', '81.218.119.11', '195.46.39.39', '107.150.40.234', '208.76.50.50', '216.146.35.35', '37.235.1.174', '89.233.43.71', '74.82.42.42', '109.69.8.51']
dnscnt = 1
  ...
  if (0==a[0]):
    ...
    try:
      ...
      for rdata in mx_dns_resp:
        ...
        c.execute('insert into name_to_mx_mapping values (?, ?)', (line, str(rdata.exchange).lower()))
        conn.commit()
        print 'Host', rdata.exchange, 'has preference', rdata.preference
      print mx_dns_resp[0].exchange.to_text()
    except dns.resolver.NXDOMAIN:
      print 'NXdomain handler'
      c.execute('insert into name_to_mx_mapping values (?, ?)', (line, 'NXDOMAIN'))
      conn.commit()
    except (dns.resolver.Timeout, dns.resolver.NoAnswer):
      print 'Timeout handler'
      c.execute('insert into name_to_mx_mapping values (?, ?)', (line, 'NoAnswer'))
      conn.commit()
